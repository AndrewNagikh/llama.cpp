// Process A: layers [0, layer_end) - autoregressive daemon
...
static bool forward_to_b(
    ...
    if (hidden == nullptr && n_tokens > 0) {
        fprintf(stderr, "gen_a: missing hidden state\n");
        return false;
    }
    ...
    if (!split_ab_send_hidden(b_fd, n_tokens, n_embd, layer_end, pos_start,
        ...
        fprintf(stderr, "gen_a: send hidden failed\n");
        return false;
    }
    const int64_t t1 = ggml_time_us();

    split_gen_b_resp bresp{};
    logits_out.clear();
    std::vector<float> * plogits = include_logits ? &logits_out : nullptr;
    if (!split_ab_recv_b_resp(b_fd, bresp, plogits)) {
        fprintf(stderr, "gen_a: recv b resp failed\n");
        return false;
    }

    resp.magic          = SPLIT_GEN_MAGIC;
    resp.version        = SPLIT_GEN_VERSION;
    resp.token_id       = bresp.token_id;
    resp.include_logits = include_logits ? 1 : 0;
    resp.n_vocab        = n_vocab;
    resp.ms_a_compute   = ms_a;
    resp.ms_ab_xfer     = (t1 - t0) / 1000.0;
    resp.ms_b_compute   = bresp.ms_compute;
    return true;
}
...
int main(int argc, char ** argv) {
    ...
    int ctrl_port = -1;
    int b_port    = -1;
    int layer_end = SPLIT_GEN_LAYER_END;
    ...
    if (!model) {
        fprintf(stderr, "gen_a: load model failed\n");
        return 1;
    }
    ...
    if (!ctx) {
        fprintf(stderr, "gen_a: create context failed\n");
        return 1;
    }
    ...
    if (b_fd < 0) {
        fprintf(stderr, "gen_a: connect to B failed port=%d\n", b_port);
        return 1;
    }

    const int listen_fd = split_tcp_listen(ctrl_port);
    if (listen_fd < 0) {
        fprintf(stderr, "gen_a: ctrl listen failed port=%d\n", ctrl_port);
        return 1;
    }

    fprintf(stderr, "gen_a: ready ctrl_port=%d layer_end=%d\n", ctrl_port, layer_end);
...
#endif
    if (ctrl_fd < 0) {
        fprintf(stderr, "gen_a: ctrl accept failed\n");
        return 1;
    }
    ...
    while (true) {
        ...
        if (!split_gen_recv_req(ctrl_fd, req, tokens_i32)) {
            fprintf(stderr, "gen_a: recv req failed\n");
            break;
        }
        ...
        if (req.cmd == SPLIT_GEN_CMD_SHUTDOWN) {
            ...
            resp.magic = SPLIT_GEN_MAGIC;
            resp.version = SPLIT_GEN_VERSION;
            split_gen_send_resp(ctrl_fd, resp, nullptr, 0);
            break;
        }
        ...
        if (req.cmd == SPLIT_GEN_CMD_PREFILL) {
            ...
            fprintf(stderr, "gen_a: unknown cmd %u\n", req.cmd);
            break;
        }
        ...
        if (decode_rc != 0) {
            fprintf(stderr, "gen_a: decode failed cmd=%u\n", req.cmd);
            break;
        }
        ...
        if (!split_gen_send_resp(ctrl_fd, resp, logits.empty() ? nullptr : logits.data(), n_vocab)) {
            fprintf(stderr, "gen_a: send resp failed\n");
            break;
        }
    }
